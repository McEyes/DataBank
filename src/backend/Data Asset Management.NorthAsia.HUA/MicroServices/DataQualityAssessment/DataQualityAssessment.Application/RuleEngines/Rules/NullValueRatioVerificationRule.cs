...
namespace DataQualityAssessment.Application.RuleEngines.Rules
{
    ...
    /// 空值比例规则
    /// </summary>
    public class NullValueRatioVerificationRule : VerificationBaseRule
    {
        private readonly ILogger<NullValueRatioVerificationRule> _logger;
        protected override RuleValidateType ValidateType { get => RuleValidateType.NullValueRatio; }

        public NullValueRatioVerificationRule(RuleDataContext context) : base(context)
        {
            _logger = App.GetService<ILogger<NullValueRatioVerificationRule>>();
        }
        ...
        public override async Task<double> EvaluateAsync(CancellationToken token)
        {
            ...
            if (columnList is List<MetadataColumnModel> { Count: > 0 })
            {
                    ...
                    using (var gridReader = await DbConnection.QueryMultipleAsync(queryEffective))
                    {
                        ...
                        if (nullCountEntity != null)
                        {
                            ...
                            foreach (var item in cols)
                            {
                                ...
                                {
                                    nullSum += Convert.ToInt64(nullCount);
                                }
                ...
                ulong validCount = totalCount - (ulong)nullSum;
                if (totalCount > 0)
                {
        ...
        protected override void BuildSql(string tableName, StringBuilder sqlBuilder, List<MetadataColumnModel> columnList)
        {
            ...
            switch (DbSetting.DbType)
            {
                case DatabaseType.SqlServer:
                    ...
                    break;
                case DatabaseType.Mysql:
                    sqlBuilder.AppendLine(" SELECT " + string.Join(",", sql) + $" FROM `{tableName.Replace("`", "")}`;");
                    break;
                case DatabaseType.Postgresql:
                    sqlBuilder.AppendLine(" SELECT " + string.Join(",", sql) + $" FROM \"{tableName}\";");
                    break;
