    ...
    public class DynamicRouteMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (routInfo != null)
            {
                ...
                var deviceName = context.Request.Headers["User-Agent"].ToString();

                // 生成唯一的缓存键
                var cacheKey = $"RateLimit:{path}:{ip}:{deviceName}";

                // 获取当前时间
                var currentTime = DateTime.UtcNow;

                // 获取缓存中的访问记录
                var accessRecords = _cache.Get<List<DateTime>>(cacheKey) ?? new List<DateTime>();

                // 移除超过60秒的记录
                accessRecords.RemoveAll(t => (currentTime - t).TotalSeconds > 60);

                // 检查访问次数是否超过限制
                if (accessRecords.Count >= routInfo.Limit)
                {
                ...
                _cache.Set(cacheKey, accessRecords, new DistributedCacheEntryOptions
                {
                    AbsoluteExpirationRelativeToNow = TimeSpan.FromMinutes(1)
                });

                // 检查参数
                if (!RequestValidator.Validate(context.Request, routInfo.ReqJson))
                {
