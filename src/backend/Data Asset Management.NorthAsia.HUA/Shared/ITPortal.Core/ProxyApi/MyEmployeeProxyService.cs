...
namespace ITPortal.Core.ProxyApi
{
    ...
    public class MyEmployeeProxyService : BaseProxyService//: IHttpDeclarative
    {
        ...
        {
            var resultStr = await httpRemoteService.GetAsStringAsync($"{authHostUrl}/api/Employee/EmployeeDeptManager/{ntid}",
            builder => builder.SetContent(new { PageIndex = 1, PageSize = 1 }, "application/json;charset=utf-8")
            .AddAuthentication("Bearer", GetToken()));
            IResult result = resultStr.To<Result>();
            if (result.Success)
        ...
        {
            var resultStr = await httpRemoteService.GetAsStringAsync($"{authHostUrl}/api/Employee/DeptManager/{dept}",
            builder => builder.SetContent(new { PageIndex = 1, PageSize = 1 }, "application/json;charset=utf-8")
            .AddAuthentication("Bearer", GetToken()));
            IResult result = resultStr.To<Result>();
            if (result.Success)
        ...
        /// 获取部门成本中心FM
        /// </summary>
        /// <param name="ntid">员工ntid</param>
        /// <returns>成本中心FM</returns>
        public async Task<JabusEmployeeInfo> GetEmployeeFMAsync(string ntid)
        {
            var resultStr = await httpRemoteService.GetAsStringAsync($"{authHostUrl}/api/Employee/EmployeeFM/{ntid}",
            builder => builder.SetContent(new { PageIndex = 1, PageSize = 1 }, "application/json;charset=utf-8")
            .AddAuthentication("Bearer", GetToken()));
            IResult result = resultStr.To<Result>();
            if (result.Success)
            ...
            //if (ntid.IsNullOrWhiteSpace()) return null;
            //var userInfo = await GetEmployeeByNtIdOrEmailAsync(ntid);
            //return await GetCostCenterFMAsync(userInfo?.CostCenterId);
        }
        ...
        /// 根据员工所在部门部门主管信息
        /// </summary>
        /// <param name="costCenterId">部门名称，区分大小写</param>
        /// <returns>部门主管wdid</returns>
        public async Task<JabusEmployeeInfo> GetCostCenterFMAsync(string costCenterId)
        {
            var resultStr = await httpRemoteService.GetAsStringAsync($"{authHostUrl}/api/Employee/FunctionManager/{costCenterId}",
            builder => builder.SetContent(new { PageIndex = 1, PageSize = 1 }, "application/json;charset=utf-8")
            .AddAuthentication("Bearer", GetToken()));
            IResult result = resultStr.To<Result>();
            if (result.Success)
            ...
            //if (deptManager == null) return null;
            //return await Get(deptManager.plant_fm_wd_id);
        }
