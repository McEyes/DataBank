...
{
    public class DataAssetJwtHandler : AppAuthorizeHandler
    {
        private ILogger<DataAssetJwtHandler> _logger;
        private readonly IDistributedCacheService _cache;
        private readonly IEventPublisher _eventPublisher;
        private readonly TrackLogProxyService _trackLog;
        public DataAssetJwtHandler(ILogger<DataAssetJwtHandler> logger, IDistributedCacheService cache, TrackLogProxyService trackLog, IEventPublisher eventPublisher) { 
            _logger = logger;
            _cache = cache;
            _trackLog = trackLog;
        ...
        public override async Task HandleAsync(AuthorizationHandlerContext context, DefaultHttpContext httpContext)
        {
            ...
            //RouteInfo? routInfo = null;
            var isServicesAPI = false;
            try
            {
                try
                {
                    ...
                    {
                        isServicesAPI = true;
                        //routInfo = _cache.HashGet<RouteInfo>(DataAssetManagerConst.RouteRedisKey, path);
                    }
                }
                catch (Exception ex)
                {
                    var _ = SendLogEvent(context.GetCurrentHttpContext(), new RouteInfo { ApiUrl = path }, (int)sw.ElapsedMilliseconds, $"{path}, HTTP status code 401.\r\n{ex.Message}");
                    _logger.LogError($"验证权限异常：{httpContext.Request.Path},\r\n{ex.Message},\r\n{ex.StackTrace}");
                    context.Fail();
                }
                if (isServicesAPI || JWTEncryption.AutoRefreshToken(context, context.GetCurrentHttpContext()))
                {
                ...
                else
                {
                    ...
                    if (isServicesAPI)
                    {
                        var _2 = SendLogEvent(context.GetCurrentHttpContext(), new RouteInfo { ApiUrl = path }, (int)sw.ElapsedMilliseconds, $"{path}, HTTP status code 401.");
                    }
                    else
                    {
        ...
        {
            //记录api调用情况
            if (context.Request.Path.Equals("/api/TrackLog", StringComparison.CurrentCultureIgnoreCase))
            {
                return;
            }
