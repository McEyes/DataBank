...
namespace ITPortal.Core.SqlParse
{
    ...
    {
        public PostgreSqlParser(SqlSugar.DbType sugerDbType) : base(sugerDbType)
        {
        }
...
WHERE 
    ...
    obj_description(pg_class.oid, 'pg_class') AS TableComment
FROM 
    pg_tables
JOIN 
...
FROM 
    pg_views
JOIN 
    pg_class ON pg_views.viewname  = pg_class.relname
WHERE 
    schemaname NOT IN ('pg_catalog', 'information_schema')
    and schemaname = '{schemaName}';
");
