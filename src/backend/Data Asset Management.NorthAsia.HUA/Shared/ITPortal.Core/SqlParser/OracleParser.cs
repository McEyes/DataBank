...
namespace ITPortal.Core.SqlParse
{
    ...
    {
        public OracleParser(SqlSugar.DbType sugerDbType) : base(sugerDbType)
        {
        }
...
WHERE 
    OWNER = '{dbName}';");
        ...
        /// <returns></returns>
        protected override string ExtractOrderByClause(string sql)
        {
            string result = base.ExtractOrderByClause(sql);

            // 处理Oracle特有的排序语法，如NULLS FIRST/LAST
            if (!string.IsNullOrEmpty(result))
            {
                result = Regex.Replace(result, @"\s+NULLS\s+(FIRST|LAST)", "", RegexOptions.IgnoreCase);
            }
