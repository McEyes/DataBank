...
namespace ITPortal.Core.Html
{
    ...
    public class HtmlTemplateParser
    {
        ...
        /// <returns></returns>
        public bool EvaluateBoolean(string expression, JObject data)
        {
            // 处理逻辑运算符（&&、||）
            if (expression.Contains("&&"))
            {
                ...
                {
                    if (!EvaluateBoolean(part.Trim(), data))
                        return false;
                }
                return true;
            }
            ...
            if (questionMarkIndex > -1)
            {
                ...
                return EvaluateCondition(conditionPart, data);
            }
        ...
        {
            // 优先处理字符串方法（contain/startWith/endWith/equal）
            var stringOperators = new Dictionary<string, Func<string, string, bool>>
                    {
                        { "contain", (a, b) => a.Contains(b) },
                        { "startWith", (a, b) => a.StartsWith(b) },
                        { "endWith", (a, b) => a.EndsWith(b) },
                        { "equal", (a, b) => string.Equals(a, b) }
                    };
            foreach (var op in stringOperators.Keys)
            {
                ...
                if (opIndex > 0)
                {
                    ...
                    var leftValue = ParseValue(leftPart, data) as string;
                    var rightValue = ParseValue(rightPart, data) as string;

                    if (leftValue != null && rightValue != null)
                    {
                        return stringOperators[op].Invoke(leftValue, rightValue);
                    }
                    return false;
                }
            }
            ...
            {
                int opIndex = FindOperatorIndex(condition, op);// condition.IndexOf(op);
                if (opIndex > 0)
                {
                    string leftPart = condition.Substring(0, opIndex).Trim();
                    string rightPart = condition.Substring(opIndex + op.Length).Trim();
        ...
        {
            int index = 0;
            while (index <= condition.Length - op.Length)
            {
                // 检查当前位置是否匹配运算符
                bool match = true;
                for (int i = 0; i < op.Length; i++)
                {
                    if (condition[index + i] != op[i])
                    {
                        match = false;
                        break;
                    }
                ...
                {
                    // 确保运算符前后是边界（非字母数字）
                    bool isLeftBoundary = index == 0 || !char.IsLetterOrDigit(condition[index - 1]);
                    bool isRightBoundary = index + op.Length == condition.Length ||
                    !char.IsLetterOrDigit(condition[index + op.Length]);
        ...
        {
            // 逻辑：字符串字面量 // 处理字符串字面量（单/双引号）
            if ((valueStr.StartsWith("'") && valueStr.EndsWith("'")) ||
              (valueStr.StartsWith("\"") && valueStr.EndsWith("\"")))
            {
                string strVal = valueStr.Substring(1, valueStr.Length - 2);
                // 尝试解析为日期
                if (DateTime.TryParse(strVal, out DateTime dateVal))
                {
                    return dateVal; // 返回DateTime类型而非字符串
                }
                return strVal;
            }
            ...
            // 变量值解析（尝试日期转换）
            var jToken = GetValueFromData(data, valueStr);
            if (jToken != null)
            {
                var val = ConvertJTokenToObject(jToken);
                if (val is string str && DateTime.TryParse(str, out DateTime date))
                {
                    return date;
                }
                return val;
        ...
        private bool CompareValues(object left, object right, string op)
        {
            ...
            {
                int comparison = DateTime.Compare(leftDate, rightDate);
        ...
        // 辅助：根据比较结果和运算符返回布尔值
        private bool CompareResult(int comparisonResult, string op)
        {
            return op switch
            {
                "==" => comparisonResult == 0,
        ...
        private JToken GetValueFromData(JObject data, string path)
        {
            try
            {
                if (path.EndsWith(".Count",StringComparison.CurrentCultureIgnoreCase))
                {
                    var list = data.SelectToken(path.Replace(".Count", "",StringComparison.CurrentCultureIgnoreCase));
                    if (list is JArray)
                    {
