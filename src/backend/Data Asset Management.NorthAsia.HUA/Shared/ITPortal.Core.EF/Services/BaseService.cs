...
namespace ITPortal.Core.Services
{
    ...
    public abstract class BaseService<T, Tdto, KeyType> : IBaseService<T, Tdto, KeyType> where T : class, IEntity<KeyType>, new() where Tdto : class, IPageEntity<KeyType>, new()
    {
        ...
        public BaseService(ISqlSugarClient db, IDistributedCacheService cache, bool noRedis = false, bool needEventBus = false)
        {
            CurrentDb = db;
            _cache = cache;
        ...
        public virtual async Task<int> Create<TEntity>(TEntity entity, bool clearCache = true) where TEntity : class, IEntity<KeyType>, new()
        {
            ...
            //return await new Repository<TEntity>().InsertAsync(entity) ? 1 : 0;
            var result = await CurrentDb.Insertable(entity).ExecuteCommandAsync();
            if (!NoRedis)
            {
        ...
        public virtual async Task<int> Modify<TEntity>(TEntity entity, bool clearCache = true) where TEntity : class, IEntity<KeyType>, new()
        {
            ...
            if (entity is IAuditEntity<KeyType>)
            {
                ...
                createEntity.UpdateBy = CurrentUser?.UserName ?? "test";
                createEntity.UpdateTime =DateTimeOffset.Now;
            }
            if (!NoRedis)
            {
            ...
            var result = await CurrentDb.Updateable(entity).ExecuteCommandHasChangeAsync();// ? 1 : 0;
            if (result)
            {
        ...
        public virtual async Task<bool> ModifyHasChange<TEntity>(TEntity entity, bool clearCache = true) where TEntity : class, IEntity<KeyType>, new()
        {
            ...
            var result = await CurrentDb.Updateable(entity).ExecuteCommandHasChangeAsync();
            if (result)
            {
                await SendLogEvent($"{DataAssetManagerConst.RedisKey}DB:Modify", entity, typeof(TEntity));
            }
        ...
        {
            var result = await CurrentDb.Deleteable<T>().Where(it => it.Id.Equals(id)).ExecuteCommandAsync();
        ...
        {
            var result = await CurrentDb.Deleteable<TEntity>().Where(it => it.Id.Equals(id)).ExecuteCommandAsync();
            //var result = await _repository.DeleteByIdAsync(id);
        ...
        {
            var result = await CurrentDb.Deleteable<T>().In(ids).ExecuteCommandAsync();
        ...
        {
            var result = await CurrentDb.Deleteable<T>().Where(expression).ExecuteCommandAsync();
            return result > 0;
        }
        ...
        {
            var result = await CurrentDb.Deleteable<TEntity>().In(ids).ExecuteCommandAsync();
            if (!NoRedis)
            {
                foreach (var id in ids)
                {
