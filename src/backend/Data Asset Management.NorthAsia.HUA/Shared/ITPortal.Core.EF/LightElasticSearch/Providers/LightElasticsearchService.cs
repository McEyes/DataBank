...
namespace ITPortal.Core.LightElasticSearch.Providers
{
    public class LightElasticsearchService : ISingleton
    {
        ...
        public async Task<CreateIndexResponse> CreateIndex<TEntity, KeyType>() where TEntity : EntityBase<KeyType>//: class, IEntity<KeyType>
        {
        ...
        public async Task<CreateIndexResponse> CreateIndex<TEntity>() where TEntity : EntityBase<string>//: class, IEntity<string>
        {
            var indexName = GetIndexFullName<TEntity>();
            if (indexName.IsNullOrWhiteSpace() || !elasticOptions.Enabled) return new CreateIndexResponse() { Acknowledged = false };
            if (_existsIndexDictionary.ContainsKey(indexName)) return new CreateIndexResponse() { Acknowledged = true, Index = indexName };
        ...
        /// <returns></returns>
        public async Task<BulkResponse> InsertManyAsync<TEntity>(IEnumerable<TEntity> entities) where TEntity : EntityBase<string>// : class, IEntity<string>
        {
            var indexName = GetIndexFullName<TEntity>();
        ...
        /// <param name="entities"></param>
        /// <returns></returns>
        public async Task<BulkResponse> InsertManyAsync<TEntity, KeyType>(IEnumerable<TEntity> entities) where TEntity : EntityBase<KeyType>//: class, IEntity<KeyType>
        {
            var indexName = GetIndexFullName<TEntity>();
            if (indexName.IsNullOrWhiteSpace() || !elasticOptions.Enabled) return new BulkResponse() { Errors = false };
            return await _client.IndexManyAsync(entities, indexName);
        ...
        /// <returns></returns>
        public async Task<IndexResponse> InsertAsync<TEntity>(TEntity entity) where TEntity : EntityBase<string>//: class, IEntity<string>
        {
            var indexName = GetIndexFullName<TEntity>();
        ...
        /// <param name="entity"></param>
        /// <returns></returns>
        public async Task<IndexResponse> InsertAsync<TEntity, KeyType>(TEntity entity) where TEntity : EntityBase<KeyType>//: class, IEntity<KeyType>
        {
            var indexName = GetIndexFullName<TEntity>();
            if (indexName.IsNullOrWhiteSpace() || !elasticOptions.Enabled) return new IndexResponse() { Result = Elastic.Clients.Elasticsearch.Result.NoOp };
            return await _client.IndexAsync(entity, (IndexName)indexName);
        ...
        public async Task<ElasticsearchResponse> UpdateAsync<TEntity>(TEntity entity) where TEntity : EntityBase<string>//: class, IEntity<string>
        {
            return await UpdateAsync<TEntity, string>(entity);
        }


        public async Task<ElasticsearchResponse> UpdateAsync<TEntity, KeyType>(TEntity entity) where TEntity : EntityBase<KeyType>// : class, IEntity<KeyType>
        {
            var indexName = GetIndexFullName<TEntity>();
