    ...
    public static class StringExtensions
    {
        ...
        public static bool IsIdentityCardId(this string value)
        {
            ...
            if (value.Length == 15)
            {
                ...
                return DateTimeOffset.TryParse(string.Format("{0}-{1}-{2}", "19" + array[2], array[3], array[4]), out _);
            }
            ...
            array = regex.Split(value);
            if (!DateTimeOffset.TryParse(string.Format("{0}-{1}-{2}", array[2], array[3], array[4]), out _))
            {
                return false;
            }
        ...
        public static T FromJsonString<T>(this string? json)
        {
            ...
            return JsonConvert.DeserializeObject<T>(json, GlobalJsonJsonConverters());
        }
        ...
        /// <param name="json">JSON字符串</param>
        /// <returns></returns>
        public static T To<T>(this JsonToken? json)
        {
            var jsonStr = json?.ToString();
            if (jsonStr.IsNullOrWhiteSpace()) return default(T);
        ...
        {
            return JsonConvert.DeserializeObject(json, type, GlobalJsonJsonConverters());
        }
        ...
        /// <param name="obj"></param>
        /// <returns></returns>
        public static string ToJSONString1(this object obj)
        {
            if (obj == null)
            {
                return null;
        ...
        #endregion
        private static JsonConverter[] GlobalJsonJsonConverters()
        {
            // 创建全局默认的序列化设置
            var list = new List<JsonConverter>();
            list.Add(new DefaultDateConverter());
            list.Add(new UtcDateTimeOffsetConverter());
            list.Add(new UtcDateTimeOffset2Converter());
            list.Add(new DefaultDateOnlyConverter());
