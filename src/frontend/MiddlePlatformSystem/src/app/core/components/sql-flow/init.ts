...
export class DataLineageService implements OnInit {
  ...
  nodeRelationFlag: any[] = [] // 判断是有关系节点，没有则连接头部
  constructor(
    private readonly dataTopicStoreHttpService: DataTopicStoreHttpService // 容器选择器
  ) { }
  ...
  public async initDataLineage(id: string, useLocalData: boolean = true): Promise<void> {
    ...
    if (useLocalData) {
      ...
      const res = await this.dataTopicStoreHttpService.getLineageData({ name: 'topic' + id })

      this.jsonData = await this.buildJSONData(res?.data || null)
      setTimeout(() => {
        this.drawData(this.jsonData);
      }, 500)
  ...
  private buildJSONData(data: any) {
    ...
    this.nodeRelationFlag = []
    // 添加关系 表和目标节点的列 
    let targetColumns: any[] = []
    data?.columnlineages.forEach((item: any) => {
      result.relations.push({
        "source": {
          "column": item.inputFields[0].field?.split(".").join(""),
          "parentName": item.inputFields[0]?.dataset?.split(".").join("") // 对应为id
        },
        "target": {
          "column": item.name?.split(".").join(""),
          "parentName": data?.graph[0]?.data?.outputs[0]?.name?.split(".").join("") // 对应为id
        }
      })

      // 记录表节点 有关系的，判断头部连接
      this.nodeRelationFlag.push(item.inputFields[0]?.dataset?.split(".").join(""))

    })

    // 生成目标节点的列
    const outDataset = data?.datasets?.find((item: any) => {
      return item.name === data?.graph[0]?.data?.outputs[0]?.name
    })
    outDataset?.fields?.forEach((item: any) => {
      targetColumns.push(
        {
    ...
    if (result.relations.length === 0) {
      result.relations[0] = {
        target: {
          parentName: data?.graph[0]?.data?.outputs[0]?.name?.split(".").join(""),
        }
      }
    }

    let originColumns: any[] = []
    data?.graph[0]?.data?.inputs.forEach((item: any, i: number) => {
      let columns: any[] = []

      // 找到表的相关信息
      const dataset = data?.datasets?.find((cItem: any) => {
        return item.name === cItem.name
      })
      ...
      if (columns.length === 0) {
        ...
        columns.unshift({
          id: 'owner',
          name: 'Owner: ' + (dataset.owner || '')
        })
        columns.unshift({
          id: 'desc',
          name: 'Desc: ' + (dataset.description || '')
        })
      }

      // 加入Origin应用节点和表对应
      if (dataset.app_source) {

        // originColumns.push({
        //   id: dataset.app_source?.split(".").join(""),
        //   name: dataset.app_source?.split(".").join(""),
        //   connect: item.name?.split(".").join(""),
        // })

        result.nodes.push({
          "id": dataset.app_source?.split(".").join(""),
          "name": dataset.app_source?.split(".").join(""),
          "description": dataset.app_source?.split(".").join(""),
          "type": "Origin",
          "connect": item.name?.split(".").join(""),
          "columns": [],
          "top": i === 0 ? 50 + 200 : 250 + 155 * i,
          "left": 150 + 300
        })
      }
    ...
    // 最后一个RS节点
    result.nodes.push({
      "id": data?.graph[0]?.data?.outputs[0]?.name?.split(".").join(""),
      "name": data?.graph[0]?.data?.outputs[0]?.name?.split(".").join(""),
      "type": "RS",
      "description": data?.graph[0]?.data?.outputs[0]?.description,
      "columns": targetColumns,
      "top": 155 * ((data?.graph[0]?.data.inputs.length + 1) / 2) - 100 + 200,
      "left": 550 + 600
    })
    ...
    const templateMap: Record<string, string> = {
      Origin: ` <div class="pa" id='{{id}}' style='top:{{top}}px;left:{{left}}px'>
        <div class="panel panel-node panel-node-origin" id='{{id}}-inner'>
            <div id='{{id}}-heading' title='{{description}}' data-id="{{id}}" class="table-header">{{name}}</div>
      ...
      Middle: `<div class="pa" id='{{id}}' style='top:{{top}}px;left:{{left}}px'>
        <div class="panel panel-node panel-node-middle" id='{{id}}-inner'>
            <div id='{{id}}-heading' title='{{description}}' title='{{description}}' data-id="{{id}}" class="table-header" style="background-color: #b24b78;color: white">{{name}}</div>
            <ul id='{{id}}-cols' class="col-group"></ul>
        </div>
    </div>`,
      UNION: `<div class="pa" id='{{id}}' style='top:{{top}}px;left:{{left}}px'>
        <div class="panel panel-node panel-node-union" id='{{id}}-inner'>
            <div id='{{id}}-heading' title='{{description}}' data-id="{{id}}" class="table-header" style="background-color: #66ccff;color: white">{{name}}</div>
            <ul id='{{id}}-cols' class="col-group"></ul>
        </div>
    </div>`,
      RS: `<div class="pa" id='{{id}}' style='top:{{top}}px;left:{{left}}px'>
        <div class="panel panel-node panel-node-rs" id='{{id}}-inner'>
            <div id='{{id}}-heading'  title='{{description}}' data-id="{{id}}" class="table-header" style="background-color: #ff9900;color: white">{{name}}</div>
            <ul id='{{id}}-cols' class="col-group"></ul>
        </div>
    </div>`
    };
  ...
  private addNodeColumns(node: Node): void {
    node.columns.forEach(col => {
      ...
      const li = $(`<li class='panel-node-list ${col.id}' title='${col.description}'></li>`);
      // 列的唯一 ID（格式：节点名.列名）
  ...
  private addOriginEndpoints(node: Node): void {
    ...
    this.jsPlumbInstance.addEndpoint(node.id + '-heading', {
      anchors: 'Right', // 锚点在左侧
      uuid: `${node.id}-heading-Right`
    }, config);
  ...
  private connectRelations(nodes: Node[], relations: Relation[]): void {
    nodes.forEach(node => {
      ...
      if (node.id !== 'RS' && node.type !== 'RS') {
        ...
        // 没有字段关系则添加表头部和主题头部相连
        if (!this.nodeRelationFlag.includes(node.id) && relations[0]?.target && node.type === 'Middle') {
          // 如果有nodeid匹配的关系 则增加连接头部信息
          const sourceUuid = `${node.id}-heading-Right`;
          const targetUuid = `${relations[0].target.parentName}-heading-Left`;
          this.connectEndpoints(sourceUuid, targetUuid);
        }

        // orgApp头部连接表头部 && node?.connect
        if (node.type === 'Origin' && node?.connect) {
          const sourceUuid = `${node.id}-heading-Right`;
          const targetUuid = `${node?.connect}-heading-Left`;
          this.connectEndpoints(sourceUuid, targetUuid);

          // node.columns.forEach(col => {
          //   const currentColId = `${node.id}.${col.id}-Right`;
          //   const relationSourceId = `${col?.connect}-heading-Left`;
          //   this.connectEndpoints(currentColId, relationSourceId);
          // });
        }

        node.columns.forEach(col => {
          const currentColId = `${node.id}.${col.id}`;
          relations.forEach(relation => {
            // 没有关系时  会自定义target name 不会有source 所以要加判断
            if (relation?.source?.parentName && relation?.source?.column) {
