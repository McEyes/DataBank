...
@Component({
  selector: 'app-data-topic-record',
...
export class DataTopicRecordComponent implements OnInit {
  ...
  constructor(
    ...
    if(this.urlParams['topic_id']) {
      this.openDetail({
        id: this.urlParams['topic_id'],
        progress: this.urlParams['progress']
      }, 'edit')
    }
  }
  ...
  async queryTable() {
    this.loading = true;
    const res = await this.dataTopicStoreHttpService.getTopicDraftData(
      this.listForm
    )
    if (res.succeeded) {
